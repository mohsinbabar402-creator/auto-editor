...
with sync_playwright() as pw:
    ...
    # Click plus button directly without focusing textarea first
    ...
    upload_btn = page.locator('button:has-text("Upload files"), [aria-label*="Upload files" i]').first
    print("Is upload_btn visible:", upload_btn.is_visible())
    if upload_btn.is_visible():
        try:
            with page.expect_file_chooser(timeout=8000) as fc_info:
                upload_btn.click()
            fc_info.value.set_files(str(vid))
            print("SUCCESS: File set via file chooser!")
        except Exception as e:
            print("File chooser error:", e)

    page.wait_for_timeout(6000)
    page.screenshot(path="whop-editor/data/analysis/test_file_chooser_attached.png")
    ctx.close()
